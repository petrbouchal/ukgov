...
for(i in jdata$results) {
  ...
  ddata <- fromJSON(getURI(paste0(apibase,'/rest/dataset/',i)))
  # write into database of datasets here
  datarow <- ddata
  datarow$extras <- NULL
  datarow$resources <- NULL
  datarow <- append(datarow,ddata$extras)
  datarow <- as.data.frame(t(datarow))
  if(firstdataset) db_datasets <- datarow else db_datasets <- smartbind(db_datasets,datarow)
  num <- 1
  firstdataset <- FALSE
