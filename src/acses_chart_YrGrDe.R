...
ac_ch$count[ac_ch$Gender==2010] <- -ac_ch$count[ac_ch$Date==2010]
...
fontfamily = 'Calibri'
plotname <- './graphs/ACSES graphs/plot_YrGrDe.pdf'
plot_YrGrDe <- ggplot(ac_ch, aes(Civil.Service.grad, share)) +
  geom_bar(position='identity', width=1, aes(fill=as.factor(Date)),stat='identity') +
#  geom_line(aes(group=grp, col=Gender), size=2) +
#  geom_area(aes(group=grp, fill=as.factor(Date)), data=ac_ch[ac_ch$Date=='2010',]) +
