...
plot_PSE <- ggplot(data=uu,aes(x=Period,y=value, group=group, colour=Whitehall)) + 
  geom_rect(data=uu2[uu2$Period=='2013Q1' & uu2$Whitehall=='Non-Whitehall Civil Service',],fill=HLcol,
  ...
  geom_rect(data=uu2[uu2$Period=='2013Q1' & uu2$Whitehall=='Non-Whitehall Civil Service',],colour=HLmarg,
            xmin = -Inf,xmax = Inf,
