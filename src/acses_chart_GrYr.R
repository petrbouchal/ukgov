...
origdata <- LoadAcsesData(file_name=filename,location='home')
...
plot_GrYr <- ggplot(uu, aes(x=Civil.Service.grad, y=share)) +
  geom_bar(position='dodge', width=.9, aes(fill=as.factor(Date)),stat='identity') +
