...
```{r fig.width=10, fig.align='center', fig.height=4, echo=FALSE}
plot
```
...
```{r fig.width=10, fig.height=15, fig.align='center', echo=FALSE}
plot2
```


```{r tidy=FALSE}
SumDeptsFTEAdj <- read.csv("~/GitHub/Charting-government/data/SumDeptsFTEAdjusted.csv")
# get rid of fluff in colnames, making it suitable for reshape and googlevis
...
library(scales)
plotFTE <- ggplot(SumDeptsFTEAdjlong, aes(x=Date,
                                   y=FTE/1000,
                                   fill=factor(Whitehall))) + 
                geom_area() +
                facet_wrap(~ Dept, ncol=3, scales = "free_y") +
